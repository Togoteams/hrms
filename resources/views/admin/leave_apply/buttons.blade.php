<form id="edit{{ $item->id }}" action="{{ route('admin.' . $route . '.destroy', $item->id) }}">
    ...
    @if (($item->status == 'approved' && $item->is_reversal == 0 && $item->user?->employee?->employment_type == 'local') && $item->approval_authority == auth()->user()->id ||
    auth()->user()->role_slug == 'managing-director' ||
    auth()->user()->id == 1)
        @if ($item->leave_type->slug == 'leave-without-pay')
            <button type="button" onclick="reverseLeaveWithoutPay({{ $item->id }})" href="#"
                class="btn btn-edit btn-sm"><i class="fa fa-undo" aria-hidden="true"></i>
            </button>
        @endif
    @endif
    @can('leave-approval')
        ...
        @else
                <button type="button" class="btn @if ($item->status == 'pending') btn-warning @elseif ($item->status == 'reject') btn-danger @elseif($item->status == 'approved') btn-success @else btn-secondary @endif btn-sm">
                    {{ ucfirst($item->status) }} </button>
        @endif
    @endcan
